...
RESNEXT_3D_PATH = '/unsullied/sharefs/jiangjianwen/workspace/code/Action_SSD_Bin/Saves/ptmodels/resnext-101-kinetics.pth'
...
class ResNeXt(nn.Module):
    ...
    def __init__(self, block, layers, sample_size, shortcut_type='B', cardinality=32, num_classes=400, last_fc=True):
        ...
        self.param_3d_2d()
        self.feature_28 = nn.Sequential(self.conv1, self.bn1, self.relu, self.maxpool, self.layer1,
                                        self.layer2)
        self.feature_14 = nn.Sequential(self.layer3)
        self.feature_7 = nn.Sequential(self.layer4)

    def param_3d_2d(self):
        state_dict_2d = self.state_dict()
        state_dict_3d = torch.load(RESNEXT_3D_PATH)['state_dict']
        for k, v in enumerate(state_dict_3d):
            name = v[7:]
            param_3d = state_dict_3d[v]
            if 'conv' in name or 'downsample.0' in name:
                param_2d = param_3d.sum(dim=-3)
                state_dict_2d[name].copy_(param_2d)
            else:
                state_dict_2d[name].copy_(param_3d)
        print('load pretrained model for resnext-101-kinetics')
...
def resnext101():
    ...
    """
    model = ResNeXt(ResNeXtBottleneck, [3, 4, 23, 3], 112)
    return model
